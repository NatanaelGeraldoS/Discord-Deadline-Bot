...
if os.path.exists("./config.json"):
    ...
    configJsonTemplate = {"token": "", "prefix": "$", "dateFormat": "%d-%m-%Y, %H:%M",
                          "bannedWords": [], "adminBannedWordBypass": False, "Deadlines": []}
...
async def serverinfo(ctx):
    ...
    embed = discord.Embed(
        title="SERVER INFO", description="Here is the info I could retrieve for this server: ", color=0x000000)
    ...
    embed.add_field(name="MEMBERS BOOSTING:",
                    value=server.premium_subscription_count, inline=True)
    embed.add_field(name="ROLES:", value=rolesString[:-2], inline=False)
    embed.add_field(name="SERVER CHANNELS:",
                    value=f"{numChannels} channels ({numTextChannels} text, {numVoiceChannels} voice)", inline=False)
    embed.add_field(name="SERVER CREATED:",
                    value=f"{server.created_at.strftime(dateFormat)} UTC {serverAgeString}", inline=False)
...
async def userinfo(ctx, name=""):
    ...
    if user != None:
        embed = discord.Embed(
            title="USER INFO", description="Here is the info I could retrieve for this user: ", color=user.colour)
        embed.set_thumbnail(url=user.avatar_url)
        embed.add_field(name="NAME:", value=user.name, inline=True)
        embed.add_field(name="NICKNAME:", value=user.nick, inline=True)
        embed.add_field(name="ID:", value=user.id, inline=True)
        embed.add_field(
            name="CREATED:", value=f"{user.created_at.strftime(dateFormat)} UTC", inline=True)
        embed.add_field(
            name="JOINED:", value=f"{user.joined_at.strftime(dateFormat)} UTC", inline=True)
...
async def getdeadline(ctx):
    embed = discord.Embed(
        title="Deadline List", description="Here is a list of Deadline That You Have:", colour=ctx.author.colour)
    numbering = 1
    for deadline in deadlines:
        embed.add_field(name=str(numbering) + ". " +
                        deadline, value="Task", inline=False)
...
@bot.command(description="Add Deadline Task.")
@commands.has_permissions(administrator=True)
...
async def help(ctx):
    embed = discord.Embed(
        title="COMMAND HELP", description="Here is a list of commands and their parameters:", colour=ctx.author.colour)
    ...
    for command in bot.commands:
        ...
        if commandCanRun == True:
            ...
            embed.add_field(name=f"**{prefix}{command} {paramString}**",
...
async def refresh(ctx):
    ...
    if message != "":
        await ctx.send('@everyone \n***You Have Deadline For Today***' + message)
    with open("./config.json", "r+") as f:
        data = json.load(f)
        data["deadlines"] = deadlines
        f.seek(0)
